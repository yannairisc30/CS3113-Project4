...
void Map::build()
{
    ...
    for (int y_coord = 0; y_coord < m_height; y_coord++)
    {
        for (int x_coord = 0; x_coord < m_width; x_coord++)
        {
            ...
            float tile_gap = 0.008f; // A tiny offset (can be slightly larger if needed)


            // Otherwise, calculate its UV-coordinated
            float u_coord = (float)(tile % m_tile_count_x) / (float)m_tile_count_x + tile_gap;
            float v_coord = (float)(tile / m_tile_count_x) / (float)m_tile_count_y + tile_gap;

            // And work out their dimensions and posititions
            float tile_width = 1.0f / (float)m_tile_count_x - 2 * tile_gap;
            float tile_height = 1.0f / (float)m_tile_count_y - 2 * tile_gap;
...
bool Map::is_solid(glm::vec3 position, float* penetration_x, float* penetration_y)
{
    ...
    *penetration_y = 0;
    ...
    //if (tile != 1 && tile != 16 && tile != 17) return false;
